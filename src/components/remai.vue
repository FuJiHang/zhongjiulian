 ...
 * @Last Modified time: 2019-03-27 17:02:18
    商品卡片
 */
<template>
    <div :class="['productChild',Endian?'productChild_E':'']" >
        <div class="img" @click="toFN(product.ProductId)">
            <img :src="imgSrc+product.img">
        </div>
        <div class="des_pd" >
            <span class="des_s_pd" @click="toFN(product.ProductId)">
                {{product.ProductName}} {{product.ShortDescription}}
            </span>
            <span class="money">
                <span class="fir" @click="toFN(product.ProductId)">￥{{product.MaxSalePrice}}<i>.{{product.MinSalePrice}}</i></span>
...
<script>
    ...
    export default {
        ...
        methods:{
            toFN(ProductId){
                this.$router.push({path:'/fuProduct?id='+ProductId})
            },
            addCart(){
